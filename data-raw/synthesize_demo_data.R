...
# # load data
# pams_temp <- tempfile()
# download.file(pams_url, pams_temp)
...
# TODO: We may be able to just filter for the top 5 companies in each sector
...
final_synthesized_abcd <- pams_with_selected_emission_factors %>%
  select(
    company_id,
    name_company,
    lei,
...
readr::write_csv(
  final_synthesized_abcd,
  "~/Downloads/test_demo_data.csv"
)
