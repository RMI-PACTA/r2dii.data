library(rlang)  # (version 0.4.5.900)
library(r2dii.utils)  # (version 0.0.0.9003)

#' Add "market share" columns to a `scenario` dataset
#'
#' @source r2dii.scenario at version 0.0.0.9002
#'   <https://github.com/2DegreesInvesting/r2dii.scenario/releases/tag/0.0.0.9002>
#'
#' @param scenario A scenario dataframe like [raw_scenario_demo].
...
#'   * `tmsr` - The "Technology Market Share Ratio", used to calculate scenario
#'   efforts for high-carbon technologies. The portfolio efforts are calculated
#'   as: \eqn{p(t) = p(0) * tmsr}, where `p(0)` is the technology-level value at
#'   `start_year`.
#'   * `smsp` - The "Sector Market Share Percentage", used to calculate scenario
#'   efforts for low-carbon technologies. The portfolio efforts are calculated
#'   as \eqn{p(t) = p(0) + (P(0) + smsp}, where where `p(0)` and `P(0)` is are
